...
export function createAlertRouter(redisClient: Redis) {
   ...
   */
  router.get('/:placeId', authenticateUser, async (req: Request, res: Response, next: NextFunction) => {
    try {
   ...
   */
  router.get('/subscription/:placeId', authenticateUser, async (req: Request, res: Response, next: NextFunction) => {
    try {
   ...
   */
  router.post('/subscription/:placeId', authenticateUser, async (req: Request, res: Response, next: NextFunction) => {
    try {
   ...
   */
  router.delete('/subscription/:placeId', authenticateUser, async (req: Request, res: Response, next: NextFunction) => {
    try {
      const subscription = await redisClient.get(
        `alert:subscription:${req.user.id}:${req.params.placeId}`
      );
