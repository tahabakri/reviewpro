...
// Define interface for requests that have the user property
...
export function createAlertRouter(redisClient: Redis) {
   ...
   */
  const getSubscription: RequestHandler = async (req: AuthRequest, res: Response) => {
    try {
   ...
   */
  const createSubscription: RequestHandler = async (req: AuthRequest, res: Response) => {
    try {
   ...
   */
  const deleteSubscription: RequestHandler = async (req: AuthRequest, res: Response) => {
    try {
      const subscription = await redisClient.get(
        `alert:subscription:${req.user.id}:${req.params.placeId}`
      );
