  ...
  try {
    ...
    const trends = await Promise.all(
      competitors.map(async (competitor) => {
        ...
        // Group by interval
        const groupedData = data.reduce((acc: Record<string, number[]>, item: ReviewData | SentimentData) => {
          let key;
          ...
          if (!acc[key]) acc[key] = [];
          acc[key].push(metric === 'sentiment'
            ? ('score' in item ? item.score : 0)
        ...
        const averages = Object.entries(groupedData).map(([period, values]) => ({
          period,
          value: Number((values.reduce((sum: number, v: number) => sum + v, 0) / values.length).toFixed(2)),
        }));
