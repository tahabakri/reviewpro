...
import type { MockInstance, MockableFunction } from '@jest/globals';
...
type WSEventFn = {
  (event: 'message', listener: WSMessageHandler): void;
  (event: 'close', listener: WSCloseHandler): void;
  (event: 'error', listener: WSErrorHandler): void;
};
...
export interface MockWebSocket extends Omit<WS, keyof WS> {
  ...
  close: MockInstance<WSCloseHandler>;
