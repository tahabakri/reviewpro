...
type SendMailFunction = (options: SendMailOptions) => Promise<SentMessageInfo>;

interface MockTransporter {
  verify: jest.MockedFunction<VerifyFunction>;
  sendMail: jest.MockedFunction<SendMailFunction>;
}

// Create mock transporter
const mockTransporter: MockTransporter = {
  verify: jest.fn<ReturnType<VerifyFunction>, Parameters<VerifyFunction>>()
    .mockResolvedValue(true),
  sendMail: jest.fn<ReturnType<SendMailFunction>, Parameters<SendMailFunction>>()
    .mockResolvedValue({ messageId: 'test-id' } as SentMessageInfo)
};
