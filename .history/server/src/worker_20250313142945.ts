...
import { InAppNotification, ProcessedReview } from './types';

// Process ETL jobs
queues.etl.process(async (job) => {
  const { reviews, entityId } = job.data;
  return await etlPipeline.processReviews(reviews, entityId);
});
...
queues.sentiment.process(async (job) => {
  ...
  await etlPipeline.checkAlerts(entityId, 'sentiment_drop');
...
// Helper functions
async function storeInAppNotification(userId: string, notification: InAppNotification): Promise<void> {
  // TODO: Add implementation for storing notifications in the database
