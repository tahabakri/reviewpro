...
import { TimelineData } from '../../types/sentiment';
...
export const RealtimeSentimentDashboard: React.FC<Props> = ({ placeId }) => {
  ...
  const timelineData = useMemo<TimelineData[]>(() => {
    ...
    const data = new Array(24).fill(0).map((_, i) => ({
      date: new Date(now - (23 - i) * hourMs).toISOString(),
      value: 0,
      volume: 0
    }));

    reviews.forEach(review => {
      const hourIndex = 23 - Math.floor((now - review.time) / hourMs);
      if (hourIndex >= 0 && hourIndex < 24) {
        data[hourIndex].value += review.sentiment.score;
        data[hourIndex].volume += 1;
      }
    });
    ...
    return data.map(point => ({
      ...point,
      value: point.volume > 0 ? point.value / point.volume : 0
    }));
  }, [reviews]);
